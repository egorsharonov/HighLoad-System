...
Для дальнейших расчётов принимается более высокое значение `606 000 000` предложений. Это консервативное допущение: оно увеличивает оценку размера хранилища и нагрузки на контур обновления данных.
...
Коэффициент суточного пика `2,5` выбран не как точное публичное значение, а как запас для проектирования. У сервиса сравнения цен трафик зависит от дневного цикла, рекламных кампаний и распродаж. Поэтому средний RPS недостаточен для выбора серверов и балансировщиков.
...
Статика и изображения должны отдаваться через CDN. Для проектирования принимается целевой cache hit ratio CDN `95%` для статических ресурсов и оптимизированных изображений. Тогда:
...
С учётом репликации Kafka с replication factor `3` внутренняя запись в брокеры создаёт примерно трёхкратный сетевой и дисковый объём:
...
Для физического размера необходимо учитывать индексы, WAL/журналы, служебные данные, реплики и запас на рост. Для PostgreSQL/Citus принимается коэффициент `2,6` к логическому объёму: `1,3` на индексы и служебные структуры и `2` на основную реплику. Тогда для таблицы предложений:
...
Такое разделение позволяет независимо настраивать TTL, rate limiting, WAF-правила и маршрутизацию.
...
DNS-балансировка подходит для выбора региона, но не обеспечивает мгновенное переключение при аварии из-за TTL и кэшей резолверов. Поэтому внутри каждого региона дополнительно используются L4/L7 балансировщики и health checks.
...
L4-слой распределяет TCP/TLS-трафик между L7-балансировщиками. L7-слой выполняет маршрутизацию по hostname и path, завершение TLS для публичного API, rate limiting, проверку заголовков и передачу запросов в Kubernetes.
...
Даже если расчёт даёт меньше трёх экземпляров, для production-режима принимается минимум три pod, чтобы их можно было распределить по трём зонам доступности.
...
Событийные таблицы не должны находиться в основной OLTP-базе. Они имеют высокий объём записи и используются для аналитики, антифрода, отчётности и пересчёта агрегатов.
...
Citus использует distribution column для распределения строк по шардам. Выбор этого столбца критичен: при правильном выборе связанные данные оказываются на одних и тех же worker nodes, что ускоряет join-запросы и уменьшает межузловой обмен [9].
...
Таблицы `merchants` и `categories` являются справочниками. Они существенно меньше таблицы предложений, поэтому размещаются как reference tables. Reference table копируется на worker nodes и может участвовать в локальных join-запросах [9].
...
Репликация задаётся на уровне таблиц ReplicatedMergeTree. В ClickHouse репликация работает на уровне отдельных таблиц, а каждый shard имеет независимую репликацию [11].
...
1. OpenSearch требует указать число primary shards при создании индекса [12];
...
Для начального этапа достаточно заложить `20 TB` полезного физического объёма под объектное хранилище. При росте каталога Ceph масштабируется добавлением OSD-узлов.
...
Redis Cluster распределяет ключевое пространство по 16 384 hash slots. Каждый master node отвечает за часть slots, а replica node может быть повышена до master при отказе [13].
...
|------------------|------------------------------------------------------------------------|------------------:|------------------:|
| PostgreSQL/Citus | base backup + WAL archiving                                            |        5–15 минут |          1–2 часа |
...
Кэш Redis и поисковый индекс OpenSearch не являются единственным источником истины. При полной потере они восстанавливаются из PostgreSQL/Citus, ClickHouse и Kafka.
...
Поиск выполняется через OpenSearch. В индекс заранее помещается документ товара с нормализованными полями:
...
Kafka использует partitions как единицу параллелизма и replication factor для отказоустойчивости [14]. Для topic `offer.updates.normalized` принимается 384 partitions. При пике 60 000 сообщений/с средняя нагрузка на partition:
...
История цены не строится из каждого пользовательского запроса. Она пересчитывается асинхронно из обновлений предложений.
...
Для ускорения проверки активных правил используется индекс:
...
| Оркестрация       | Kubernetes                                    | управление репликами, rolling updates, service discovery                       |
| Ingress           | NGINX Ingress или Envoy Gateway               | L7-маршрутизация и TLS termination                                             |
| Очереди           | Apache Kafka                                  | потоковая обработка, partitions, replication factor                            |
| OLTP              | PostgreSQL + Citus                            | SQL, транзакции, горизонтальное шардирование                                   |
| Поиск             | OpenSearch                                    | полнотекстовый поиск и фасеты                                                  |
| Аналитика         | ClickHouse                                    | колоночное хранение и быстрые аналитические запросы                            |
| Кэш               | Redis Cluster                                 | низкая задержка и распределение ключей                                         |
| Object storage    | Ceph RGW                                      | S3-совместимое open-source хранилище                                           |
| Мониторинг        | Prometheus + Grafana                          | метрики и дашборды                                                             |
| Логи              | Loki или OpenSearch Dashboards                | централизованный просмотр логов                                                |
| Трейсинг          | OpenTelemetry + Jaeger/Tempo                  | анализ распределённых запросов                                                 |
| Secrets           | HashiCorp Vault или External Secrets Operator | централизованное управление секретами                                          |
| CI/CD             | GitLab CI/CD                                  | сборка, тесты, деплой в Kubernetes                                             |

Выбор технологий ориентирован на возможность самостоятельного развёртывания. Managed-сервисы в работе не являются обязательными, так как задача курсового проекта — показать архитектуру и расчёт собственной инфраструктуры.
...
4. `PodDisruptionBudget`, чтобы voluntary disruptions не удаляли слишком много pod'ов одновременно;
5. `readinessProbe` и `livenessProbe` для исключения неготовых pod'ов из балансировки.
...
4. Kubernetes NetworkPolicy между namespace'ами;
...
| Данные               | Меры защиты                                                     |
|----------------------|-----------------------------------------------------------------|
| Пароли пользователей | Argon2id/bcrypt, соль, запрет хранения открытого пароля         |
...
1. CDN для статики и изображений;
2. rate limiting на edge-уровне;
...
Такое разделение уменьшает связанность компонентов: отказ поискового индекса не приводит к потере каталожных данных, а отказ Redis не приводит к потере пользовательского состояния.
...
Для Kubernetes используются три зоны доступности в каждом регионе. Stateful-хранилища могут разворачиваться как вне Kubernetes, так и через операторов, но в обоих случаях они должны быть распределены по зонам.
...
9 Kafka brokers с replication factor 3 позволяют распределить partitions и выдерживать отказ broker без потери доступности, если настроены `min.insync.replicas = 2` и producer `acks=all`.
...
Для изображений товаров рассчитан логический стартовый объём около `1,44 TB`, а с запасом — `5 TB`. При трёхкратной репликации и 30% запасе требуется около `20 TB` физического объёма. Группа из 6–9 OSD nodes обеспечивает запас и позволяет масштабировать объектное хранилище добавлением дисков или узлов.
...
[9] Citus Documentation: Choosing Distribution Column.  
https://learn.microsoft.com/en-us/postgresql/citus/data-modeling

[10] Citus Documentation: Co-locating tables and distributed tables.  
https://docs.citusdata.com/en/stable/develop/reference.html

[11] ClickHouse Documentation: Replication and MergeTree partitioning.  
...
[12] OpenSearch Documentation: Index settings.  
https://docs.opensearch.org/latest/install-and-configure/configuring-opensearch/index-settings/

[13] Redis Cluster Specification.  
https://redis.io/docs/latest/operate/oss_and_stack/reference/cluster-spec/

[14] Apache Kafka Documentation: replication and partitions.  
