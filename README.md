...
Для системы используются разные типы хранилищ, так как один тип базы данных не покрывает все требования одинаково эффективно. В таблице ниже указано не только назначение хранилища, но и причина выбора именно этой технологии.

| Контур                        | Хранилище                           | Почему выбран именно этот вариант                                                                                                                                                                                         |
|-------------------------------|-------------------------------------|---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------|
| Пользовательские данные       | PostgreSQL + Citus                  | PostgreSQL даёт транзакции, SQL и зрелую экосистему, а Citus добавляет горизонтальное шардирование по distribution column и co-location связанных таблиц [10][11].                                                        |
| Актуальные предложения        | PostgreSQL + Citus                  | `offers_current` — это OLTP-таблица текущего состояния, где нужны upsert, индексы и быстрый доступ по `product_id`; Citus позволяет распределить сотни миллионов строк по worker-узлам [10].                              |
| Поиск                         | OpenSearch                          | Поиск требует полнотекстового ранжирования, фасетов, фильтров и независимого масштабирования индекса через primary/replica shards [13]. PostgreSQL остаётся source of truth, но не обслуживает тяжёлый поиск по каталогу. |
| События и аналитика           | ClickHouse                          | События и временные ряды имеют append-only профиль и требуют агрегаций по большим объёмам. ClickHouse с MergeTree, партиционированием по времени и репликацией подходит для таких OLAP-запросов лучше OLTP-БД [12][27].   |
| Кэш и rate limiting           | Redis Cluster                       | Redis даёт низкую задержку, TTL, атомарные Lua-операции и распределение ключей по 16 384 hash slots [14]. Это удобно для кэша, коротких счётчиков и token bucket rate limiting [35].                                      |
| Очереди и потоковая обработка | Apache Kafka                        | Kafka хранит события в partitioned log, поддерживает replay, consumer groups, replication factor и межкластерную репликацию через MirrorMaker 2 [15][26][38].                                                             |
| Объектное хранилище           | Ceph Object Gateway (RADOS Gateway) | Ceph RGW предоставляет S3-совместимый API поверх собственного Ceph-кластера, масштабируется добавлением OSD-узлов и поддерживает multisite-репликацию [8][23][37].                                                        |
| Авторизационные сессии        | PostgreSQL + Redis                  | Долгоживущие refresh-сессии и token family хранятся в PostgreSQL как источник истины, а Redis используется только для краткоживущего кэша отозванных session/version и защиты от частых проверок.                         |

MinIO не выбирается как основное S3-хранилище, так как репозиторий `minio/minio` был архивирован владельцем 13 февраля 2026 года и переведён в read-only режим [9]. Для курсового проекта допустимо рассмотреть Garage как более лёгкую альтернативу, но для данного масштаба выбран Ceph Object Gateway: он лучше подходит для промышленного объектного хранилища, поддерживает S3-совместимый API и может использоваться поверх Ceph Storage Cluster [8][23].
